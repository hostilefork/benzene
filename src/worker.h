...
private:
    ...
    optional<methyl::RootNode<methyl::Node>> _root;
...
private:
    ...
    // on a Node then that Observer will get the observation.  The context
...
private:    
    ...
    // a move-only RootNode.  When we pass them back into client code, we
    // send non-owned NodeRefs.  The ordered sequence is stored in _hitList
    // while they are kept from being freed by _ownedHits.

    std::unordered_set<methyl::RootNode<Hit>> _ownedHits;

    std::vector<optional<methyl::NodeRef<Hit const>>> _hitList;
...
private:
    void addHitHelper (optional<methyl::RootNode<Hit>> hit);
